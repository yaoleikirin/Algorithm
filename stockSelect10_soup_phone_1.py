...
class getstock:
    ...
    def history_from(self,day_from): #change name to history
        #print("发行日期:",self.date) 
        #day_from  #<class 'datetime.date'>
        #如果开始日期比上市日期还早
        if day_from<datetime.date(*map(int,self.date.split('-'))):
            date_from=self.date
        else:
            date_from=str(datetime.datetime.strptime(str(day_from),'%Y-%m-%d')).split(' ')[0]
        styear,stmonth,stday=date_from.split('-')
        stseason=int(stmonth)//3+1
        if stseason==5:
            stseason=4
        curyear,curmonth,curday,hour,minute,second=Today.split('-')
        curseason=int(curmonth)//3+1
        if curseason==5:
            curseason=4
        year=int(curyear)
        season=int(curseason)        
    ...
    def dayN(self,N):
        #N+60 如果从指定日数倒推，可能得不到N个数据，所以偏移60
        day_from = datetime.datetime.now().date() - datetime.timedelta(days=N+60) #<class 'datetime.date'>
        df=self.history_from(day_from)
        df.sort_index(inplace=True)
        return df.tail(N)
